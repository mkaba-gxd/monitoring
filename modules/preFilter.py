    ...
    for pj_type in df_info['PRJ_TYPE'].unique():
        ...
        for i, item in df_prj.iterrows() :
            ...
            if pj_type == 'eWES':
                ...
                data['REPORT'] = np.where(data['Clinvar_CLNSIG'].str.contains('Pathogenic', case=True, regex=False),'PASS',
                                np.where(data['Clinvar_CLNSIG'].str.contains('Likely_pathogenic', case=True, regex=False),'PASS',
                                np.where(data['ONCOKB_ONCOGENICITY'].str.contains('Oncogenic', case=True, regex=False),'PASS',
                                np.where(data['ONCOKB_ONCOGENICITY'].str.contains('Likely Oncogenic', case=True, regex=False),'PASS',''))))
