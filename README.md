...
指定された sample ID や flowcell ID をデータベースに問い合わせ、抽出した検体情報を基に解析ディレクトリ内のデータを加工します。そのため**データベース未登録の検体**や、**規程の場所に中間ファイルがない検体**に対しては実行できません。\
**このツールは正式な検証を経ていません。** 不具合等が生じた場合は適宜修正するか、手作業で必要なデータを解析フォルダから抽出・加工してください。\
