...
指定されたsample IDやflowcell IDからデータベースで検索した検体情報を基に解析ディレクトリ内のデータをロードするため、**データベース未登録の検体**や、**規程の場所に中間ファイルがない検体**に対しては実行できません。\
なお、**データベースの設計内容が不明なため、データベース検索時に想定外の動作を行う可能性があります。**\
また、**このツールは正式な検証を経ていません。** 不具合等が生じた場合は適宜修正するか、手作業で必要なデータを解析フォルダから抽出・加工してください。
